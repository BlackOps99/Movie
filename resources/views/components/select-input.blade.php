...
<div x-data="select({{ $json }})" x-init="init()" @click.away="closeListbox()" @keydown.escape="closeListbox()"
    class="relative">
    ...
    <span class="">
        <button type="button" x-ref="button" @click="toggleListboxVisibility()" :aria-expanded="open"
            ...
            class="relative focus:shadow-soft-primary-outline text-sm leading-5.6 ease-soft block w-full appearance-none rounded-lg border border-solid border-gray-300 bg-white bg-clip-padding px-3 py-2 font-normal text-gray-700 transition-all focus:border-fuchsia-300 focus:outline-none focus:transition-shadow">
        ...
        <ul x-ref="listbox" @keydown.enter.stop.prevent="selectOption()"
            ...
            <template x-for="(key, index) in Object.keys(options)" :key="index">
                <li :id="name + 'Option' + focusedOptionIndex" @click="selectOption()"
                    ...
                    :class="{
                        'text-red-500 bg-indigo-600': index ===
                            focusedOptionIndex,
    ...
    function select(config) {
        return {
            ...
            init: function() {
                ...
                document.getElementById('movie_id').value = this.value;
